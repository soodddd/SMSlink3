...
fun PairingDialog(
    ...
    val scanLauncher = rememberLauncherForActivityResult(
        ...
        if (scanResult.contents?.isNotBlank() == true) qrCodeInput = scanResult.contents
    }
    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
                ...
                Button(onClick = scanQrCode, modifier = Modifier.fillMaxWidth()) {
                    Text("扫描二维码")
                }
                ...
                if (qrCode != null) {
                    Card(
                        ...
                        val qrBitmap = remember(qrCode) {
                            runCatching { createPairingQrBitmap(qrCode.orEmpty()) }.getOrNull()
                        }
                        if (qrBitmap != null) {
                            androidx.compose.foundation.Image(
                                bitmap = qrBitmap.asImageBitmap(),
                                contentDescription = "SMS-link pairing QR code",
