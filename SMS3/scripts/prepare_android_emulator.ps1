param(
    ...
    [int]$AdbWaitTimeoutSec = 10,
    [int]$BootTimeoutSec = 60,
    [int]$PackageManagerTimeoutSec = 30,
    [int]$AppLaunchTimeoutSec = 20,
    [int]$ProcessStableTimeoutSec = 15,
...
# Wait for ADB connection with short timeout
$adbReady = Wait-Until -TimeoutSec $AdbWaitTimeoutSec -Condition {
    $state = Invoke-AdbText @("get-state")
    $state -match "device"
}
if (-not $adbReady) {
    $result.classification = "adb_blocker"
    $result.notes += "adb wait-for-device timed out after $AdbWaitTimeoutSec seconds"
...
if ($bootReady) {
    $pmReady = Wait-Until -TimeoutSec $PackageManagerTimeoutSec -Condition {
    ...
    if (-not $pmReady) {
        $result.classification = "platform_boot"
        $result.notes += "package manager was not ready after $PackageManagerTimeoutSec seconds"
    }
}
...
if ($ActivityComponent -and $result.packageResolvable -ne $false) {
    ...
    $launchDuration = ((Get-Date) - $launchStart).TotalSeconds

    $result.appLaunchSucceeded = ($LASTEXITCODE -eq 0 -and $launch -match "Status:\s*ok")
    if (-not $result.appLaunchSucceeded) {
        $result.classification = "platform_launch"
        $result.notes += "activity launch did not report Status: ok (took $([math]::Round($launchDuration, 1))s)"
    }

    if ($result.appLaunchSucceeded) {
        $launched = Wait-Until -TimeoutSec $ProcessStableTimeoutSec -SleepSec 2 -Condition {
            $appProcessId = Invoke-AdbText @("shell", "pidof", "-s", $PackageName)
            $LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($appProcessId)
        }
        $result.appPid = Invoke-AdbText @("shell", "pidof", "-s", $PackageName)
        if (-not $launched) {
            $result.classification = "app_launch_uncertain"
            $result.notes += "process did not stabilize after $ProcessStableTimeoutSec seconds"
        }
    }
}
...
$focusText = "$($result.focusedApp)`n$($result.currentFocus)"
if ($focusText -match "isn't responding|Application Not Responding|ANR") {
    if ($focusText -match "System UI|Launcher|PermissionController|nexuslauncher") {
        $result.classification = "platform_ui"
        $result.notes += "system-owned ANR dialog detected"
    } elseif ($PackageName -and $focusText -match [regex]::Escape($PackageName)) {
